...
import { createThueGame } from "../../../services/thueGameLienHeAPI";
const CreateThueGame = ({openCreateThueGame, setOpenCreateThueGame, fetchOneThueGames}) => {

    const [form] = Form.useForm()
    const [isSubmit, setIsSubmit] = useState(false);

    const cancel = () => {
        setOpenCreateThueGame(false)
        form.resetFields()
    }
    ...
    const handleCreateThueGame = async (values) => {
        const {text} = values
        setIsSubmit(true)
        const res = await createThueGame(text)
        if(res && res.data) {
  ...
  return (
    <Modal
        ...
        title="Tạo nội dung thuê game"
        centered
        open={openCreateThueGame}
        onOk={() => form.submit()}
        onCancel={() => cancel()}
        width={800}
        okText={"Xác nhận tạo"}
